...
class MultiScaleTriplaneUNet(nn.Module):
    """
    多分辨率三平面UNet模型，支持分辨率嵌入
    """
    ...
    def __init__(
        ...
        if multiscale:
            # 时间和分辨率联合嵌入
            self.time_embed_layer = SinusoidalPosEmb(time_embed_dim // 2)
            self.scale_embed_layer = SinusoidalPosEmb(time_embed_dim // 2)
            # 专用的多分辨率time_embed
            self.time_embed = nn.Sequential(
                linear(time_embed_dim, time_embed_dim * 2),
                SiLU(),
                linear(time_embed_dim * 2, time_embed_dim),
            )
            # 同时保留单分辨率time_embed以保证兼容性
            self.time_embed_single = nn.Sequential(
                linear(model_channels, time_embed_dim),
                SiLU(),
                linear(time_embed_dim, time_embed_dim),
            )
    ...
    def forward(self, x, timesteps, scale=None, H=None, W=None, D=None, y=None):
        """
        Apply the model to an input batch.
        
        :param x: triplane features
        :param timesteps: diffusion timesteps
        :param scale: resolution scale index
        :param H, W, D: triplane dimensions
        :param y: optional class conditioning
        :return: denoised triplane features
        """
        assert H is not None and W is not None and D is not None

        hs = []
        
        # 创建嵌入 - 优化策略：降低多分辨率模式的复杂度
        if self.multiscale and scale is not None:
            # 仅在真正需要多分辨率时使用联合嵌入
            time_emb = self.time_embed_layer(timesteps)
            scale_tensor = th.ones_like(timesteps) * scale
            scale_emb = self.scale_embed_layer(scale_tensor)
            combined_emb = th.cat([time_emb, scale_emb], dim=-1)
            emb = self.time_embed(combined_emb)
        else:
            # 默认使用标准时间嵌入，与单分辨率保持一致
            if self.multiscale:
                emb = self.time_embed_single(timestep_embedding(timesteps, self.model_channels))
            else:
                emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
